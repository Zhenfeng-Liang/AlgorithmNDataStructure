...
def answer(chunk, word):
    ...
    # All the different combinations depend on the starting index of the first one
    for offset in range(0, n):
        tmpChunk = chunk
        start = tmpChunk.find(word, offset)

        while start != -1:
            tmpChunk = tmpChunk[:start] + tmpChunk[start+n:]
            start = tmpChunk.find(word)

        if tmpChunk != chunk:
            ans.append(tmpChunk)
    
    # Once there is no more match, the rest of the strings are the shortest ones, 
    # we just need to compare lexicographical order which is done by sorted function
    return sorted(ans)[0]
